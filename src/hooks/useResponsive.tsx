...
import { StackGapContext } from '../components/_stackContext'
...
// ============================================
// Responsive Override (context-based viewport simulation)
// ============================================
// When set, all responsive hooks use this width instead of window.innerWidth.
// Used by preview frames / docs to simulate viewports without resizing the real window.
const ResponsiveOverrideContext = createContext<number | null>(null)

interface ResponsiveOverrideProviderProps {
  width: number | null
  children: React.ReactNode
}

export function ResponsiveOverrideProvider({ width, children }: ResponsiveOverrideProviderProps) {
  return (
    <ResponsiveOverrideContext.Provider value={width}>
      {children}
    </ResponsiveOverrideContext.Provider>
  )
}

export function useResponsiveOverride(): number | null {
  return useContext(ResponsiveOverrideContext)
}
...
// useMediaQuery
// ============================================
// Parses `(max-width: Npx)` / `(min-width: Npx)` fragments so it can honor the
// override context. Falls back to window.matchMedia when no override is set.
function parseMediaQuery(query: string): { maxWidth?: number; minWidth?: number } {
  const result: { maxWidth?: number; minWidth?: number } = {}
  const maxMatch = query.match(/max-width:\s*(\d+)px/)
  const minMatch = query.match(/min-width:\s*(\d+)px/)
  if (maxMatch) result.maxWidth = parseInt(maxMatch[1], 10)
  if (minMatch) result.minWidth = parseInt(minMatch[1], 10)
  return result
}

function matchesWidth(width: number, parsed: { maxWidth?: number; minWidth?: number }): boolean {
  if (parsed.maxWidth !== undefined && width > parsed.maxWidth) return false
  if (parsed.minWidth !== undefined && width < parsed.minWidth) return false
  return true
}

export function useMediaQuery(query: string): boolean {
  const override = useContext(ResponsiveOverrideContext)

  const [matches, setMatches] = useState(() => {
    if (override !== null) return matchesWidth(override, parseMediaQuery(query))
  ...
  useEffect(() => {
    if (override !== null) {
      setMatches(matchesWidth(override, parseMediaQuery(query)))
      return
    }
...
export function useBreakpoint(): Breakpoint {
  ...
  const [breakpoint, setBreakpoint] = useState<Breakpoint>(() => {
    if (override !== null) return widthToBreakpoint(override)
  ...
  useEffect(() => {
    if (override !== null) {
      setBreakpoint(widthToBreakpoint(override))
      return
    }
    if (typeof window === 'undefined') return

    const updateBreakpoint = () => setBreakpoint(widthToBreakpoint(window.innerWidth))
...
export function Stack({
  ...
  return (
    <StackGapContext.Provider value={true}>
      <div style={{
        display: 'flex',

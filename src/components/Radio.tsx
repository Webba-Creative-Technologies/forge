...
export function Radio({
  ...
  const { hovered, pressed, bind } = useInteractionState({ enabled: !disabled })

  const sizeMap = {
    xs: { dimension: 14, dot: 7 },
    sm: { dimension: 16, dot: 8 },
    md: { dimension: 20, dot: 10 },
    lg: { dimension: 24, dot: 12 },
    xl: { dimension: 28, dot: 14 }
  }
  const dimension = sizeMap[size].dimension
...
function RadioCardItem({
  ...
  const { hovered, pressed, bind } = useInteractionState({ enabled: !option.disabled })

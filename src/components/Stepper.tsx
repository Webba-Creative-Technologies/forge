...
import { Checkmark16Regular, Checkmark20Regular, ChevronLeft20Regular, ChevronRight20Regular } from '@fluentui/react-icons'
...
function StepCircle({ index, isActive, completed, icon, color, size = 36, allowClick, onClick, animationDelay = 0 }: StepCircleProps) {
  const { hovered: isHovered, pressed: isPressed, bind } = useInteractionState({ enabled: allowClick })
...
// ============================================
// STEPPER DOT (dots variant indicator)
// ============================================
...
function StepperDot({ index, isActive, completed, color, allowClickNavigation, onClick }: StepperDotProps) {
  const { hovered, bind } = useInteractionState({ enabled: allowClickNavigation })
...
export function StepActions({
  ...
  return (
    <div style={{
      ...
      <div style={{ minWidth: isMobile ? 80 : 120 }}>
        {showPrev && !isFirst && (
          <Button variant="ghost" size={btnSize} onClick={prevStep} icon={<ChevronLeft20Regular />}>
            {!isMobile && prevLabel}
          </Button>
        )}
      </div>
      ...
      <div style={{ minWidth: isMobile ? 80 : 120, display: 'flex', justifyContent: 'flex-end' }}>
        {isLast ? (
          showFinish && (
            <Button variant="primary" size={btnSize} onClick={onFinish} icon={<Checkmark20Regular />}>
              {finishLabel}
            </Button>
          )
        ) : (
          showNext && (
            <Button variant="primary" size={btnSize} onClick={nextStep} disabled={isNextDisabled}>
              {nextLabel} <ChevronRight20Regular style={{ marginLeft: '0.25rem', fontSize: '1rem' }} />
            </Button>
          )
        )}
      </div>

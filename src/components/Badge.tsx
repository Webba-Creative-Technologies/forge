...
export function Badge({ children, variant = 'default', size = 'md', dot, onClose, onClick, style }: BadgeProps) {
  const isInteractive = !!(onClick || onClose)
  const { hovered, pressed, bind } = useInteractionState({ enabled: isInteractive })
  ...
  return (
    <span
      onClick={onClick}
      {...bind}
      className={onClose ? 'animate-scaleIn' : undefined}
      style={{
        display: 'inline-flex',
        alignItems: 'center',
        flex: '0 0 auto',
        ...
        paddingRight: onClose ? '0.25rem' : sizeStyles[size].paddingInline,
        ...style
      }}>
      {dot && (
        <span style={{
...
/** @deprecated Use Badge with onClose prop instead */

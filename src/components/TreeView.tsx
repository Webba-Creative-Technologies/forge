...
interface TreeViewNodeProps {
  ...
  expanded: Set<string>
  checked: Set<string>
  config: { fontSize: string; padding: string; iconSize: number; indent: number }
  showLines: boolean
  checkable: boolean
  toggleExpand: (nodeId: string) => void
  handleSelect: (node: TreeNode) => void
  handleCheck: (node: TreeNode) => void
  renderNode: (node: TreeNode, level: number, isLast: boolean, parentLines: boolean[]) => ReactNode
}
...
function TreeViewNode({
  ...
  const isChecked = checked.has(node.id)
  const { hovered, bind } = useInteractionState({ enabled: !node.disabled })

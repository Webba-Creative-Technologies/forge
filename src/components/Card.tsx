...
// CARD (Base container)
// ============================================
type CardPadding = 'none' | 'xs' | 'sm' | 'md' | 'lg' | 'xl' | '2xl' | '3xl' | '4xl'
...
interface CardProps {
  ...
  padding?: CardPadding
  variant?: 'default' | 'raised' | 'subtle' | 'outlined' | 'elevated'
  onClick?: () => void
  hoverable?: boolean
  className?: string
  style?: CSSProperties
...
export function Card({
  ...
  const { hovered: isHovered, pressed: isPressed, bind } = useInteractionState({ enabled: isInteractive })

  // Padding resolves to the unified spacing scale (--spacing-*).
  // Card-specific sm/md/lg keep slightly more generous values than raw
  // SPACING_SEMANTIC to match the "breathable card" visual language.
  const paddingStyles: Record<CardPadding, string | number> = {
    none: 0,
    xs: 'var(--spacing-xs)',  // 4px
    sm: '0.875rem',            // 14px — card-specific compact
    md: '1.25rem',             // 20px — card-specific default
  ...
  // Background based on variant. The Forge surface ladder for filled cards:
  //   default (--bg-secondary)  → page-level cards on top of --bg-primary
  //   raised  (--bg-tertiary)   → cards nested inside another card
  //   subtle  (--bg-subtle)     → very faint surface for non-essential blocks
  //   outlined (transparent)    → only when a colored/gradient bg sits behind
  //   elevated (--bg-secondary + heavy shadow + border) → lifted floating feel
  const getBackground = () => {
    if (isInteractive && isHovered) {
      // Hover always lightens by one step from current.
      if (variant === 'raised') return 'var(--bg-dropdown)'
  ...
  return (
    <div
      ...
      style={{
        ...
        border: variant === 'outlined' || variant === 'elevated' ? '1px solid var(--border-subtle)' : undefined,
        boxShadow: variant === 'elevated'
          ? '0 8px 24px rgba(0, 0, 0, 0.25), 0 2px 6px rgba(0, 0, 0, 0.15)'
          : shadows && variant === 'default' ? 'var(--shadow-card)' : undefined,
...
export function ImageCard({
  ...
  const { shadows } = useForge()
  const { hovered: isHovered, pressed: isPressed, bind } = useInteractionState({ enabled: !!onClick })
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 'var(--radius-lg)',
        boxShadow: shadows ? 'var(--shadow-card)' : undefined,
        overflow: 'hidden',
        cursor: onClick ? 'pointer' : 'default',
        transition: 'all 0.15s cubic-bezier(0.4, 0, 0.2, 1)',
...
export function ActionCard({
  ...
  const { shadows } = useForge()
  const { hovered: isHovered, bind } = useInteractionState({ enabled: !!onClick })
...
export function InfoCard({ children, title, icon, padding = 'md' }: InfoCardProps) {
  const { shadows } = useForge()
  const paddingStyles: Record<NonNullable<InfoCardProps['padding']>, string | number> = {
    none: 0,
    xs: 'var(--spacing-xs)',
    sm: '1rem',
    md: '1.25rem',

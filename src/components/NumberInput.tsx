...
export function NumberInput({
  ...
  const { focused, bind: focusBind } = useInteractionState()
  ...
  const sizeStyles = {
    xs: { height: 28, fontSize: '0.72rem',   padding: '0 0.375rem', buttonSize: 20 },
    ...
    xl: { height: 56, fontSize: '0.9375rem', padding: '0 1.25rem',  buttonSize: 48 }
  }
  ...
  if (variant === 'inline') {
    return (
      <div className={className} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, ...style }}>
        {label && (
          <span style={{ fontSize: styles.fontSize, color: 'var(--text-secondary)' }}>
            {label}

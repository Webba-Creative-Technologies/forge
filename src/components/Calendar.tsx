...
function EventHoverCard({ event, color, children, position = 'bottom' }: EventHoverCardProps) {
...
function CalendarDayCell({
  ...
  const renderEventDefault = (event: CalendarEvent) => {
    const eventColor = event.color || color
    return (
      <EventHoverCard key={event.id} event={event} color={color}>
        <div
          onClick={(e) => {
            e.stopPropagation()
            onEventClick?.(event)
          ...
          onMouseEnter={(e) => {
            e.currentTarget.style.backgroundColor = `${eventColor}35`
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        backgroundColor: isToday ? color : isSelected ? `${color}22` : 'transparent',
        color: isToday ? 'white' : isSelected ? color : 'var(--text-primary)',
        fontSize: '0.8125rem',
...
function AgendaItem({ event, color, onClick, index }: AgendaItemProps) {
  const { hovered: isHovered, bind } = useInteractionState()
  const eventColor = event.color || color
  ...
  return (
    <EventHoverCard event={event} color={color} position="top">
      <div
        onClick={onClick}
        {...bind}
        className="animate-fadeIn"
      ...
      >
        ...
        {event.icon && (
          <span style={{ color: eventColor, display: 'flex', alignItems: 'center' }}>
            {event.icon}
          </span>
        )}
        <div style={{ flex: 1 }}>
            ...
            return upcoming.map(({ date, events: dayEvents }) => (
              <div key={date.toISOString()} className="animate-fadeIn" style={{ marginBottom: '1.5rem' }}>
                ...
                <div style={{
                  fontSize: '0.8125rem',
                  fontWeight: 600,
                  color: isToday(date) ? color : 'var(--text-muted)',
                  marginBottom: '0.75rem',
                  textTransform: 'capitalize'
                }}>

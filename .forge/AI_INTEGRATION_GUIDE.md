...
| `formatValue` | `(value: number) => string` | `String` | Format value display |
| `size` | `'sm' \| 'md' \| 'lg' \| 'xl'` | `'md'` | Slider size |
| `color` | `string` | `'var(--brand-primary)'` | Track fill color |
| `marks` | `{ value: number; label?: string }[]` | - | Mark points on track |
| `showTooltip` | `boolean \| 'always'` | `true` | Show value tooltip |
...
| `onChange` | `(tags: string[]) => void` | Required | Change handler |
...
| `validateTag` | `(tag: string) => boolean \| string` | - | Validation function |
| `transformTag` | `(tag: string) => string` | - | Transform before adding |
| `separator` | `string \| RegExp` | - | Custom separators |
| `size` | `'sm' \| 'md' \| 'lg'` | `'md'` | Input size |
| `variant` | `'default' \| 'pills'` | `'default'` | Tag style |
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `tabs` | `Array<{ id: string, label: string, icon?: ReactNode, count?: number, disabled?: boolean }>` | Required | Tab items |
| `active` | `string` | Required | Active tab ID |
| `onChange` | `(id: string) => void` | Required | Change handler |
| `variant` | `'default' \| 'underline' \| 'pills'` | `'underline'` | Tab style |
| `size` | `'sm' \| 'md' \| 'lg'` | `'md'` | Tab size |
| `fullWidth` | `boolean` | `false` | Stretch tabs to full width |
| `stretchLine` | `boolean` | `false` | Stretch underline across full width |
...
| `items` | `Array<{ id: string, label: string, href?: string }>` | - | Nav items |
| `activeId` | `string` | - | Active item ID |
| `onNavigate` | `(id: string) => void` | - | Navigation handler |
| `actions` | `ReactNode` | - | Right side actions |
| `showSearch` | `boolean` | `false` | Show search button |
| `onSearchClick` | `() => void` | - | Search click handler |
| `sticky` | `boolean` | `false` | Sticky positioning |
| `transparent` | `boolean` | `false` | Transparent background |
| `variant` | `'default' \| 'centered' \| 'minimal'` | `'default'` | Navbar style |
| `forceMobile` | `boolean` | `false` | Force mobile mode |
| `forceDesktop` | `boolean` | `false` | Force desktop mode |
...
```tsx
import { Tour, TourStep } from 'wss3-forge'
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `target` | `string` | Required | CSS selector for target element |
...
| `disableInteraction` | `boolean` | `false` | Disable target interaction |
...
| `startAt` | `number` | `0` | Starting step index |
...
| `size` | `'sm' \| 'md' \| 'lg' \| 'xl' \| 'full'` | `'md'` | Sheet width |
| `width` | `number \| string` | - | Custom width override |
...
#### Affix
Sticky element that pins to viewport on scroll.

```tsx
import { Affix } from 'wss3-forge'

// Stick to top when scrolled
<Affix offsetTop={0}>
  <Navbar />
</Affix>

// Stick to bottom
<Affix position="bottom" offsetBottom={20}>
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `offsetTop` | `number` | `0` | Offset from top when affixed |
| `offsetBottom` | `number` | - | Offset from bottom when affixed |
| `position` | `'top' \| 'bottom'` | `'top'` | Affix position |
| `onChange` | `(affixed: boolean) => void` | - | Called when affix state changes |
...
| `icon` | `ReactNode` | - | Custom icon (instead of initials) |
...
| `direction` | `'row' \| 'column'` | - | Stack direction (Stack only) |
| `align` | `'start' \| 'center' \| 'end' \| 'stretch'` | - | Cross-axis alignment |
| `justify` | `'start' \| 'center' \| 'end' \| 'between' \| 'around'` | - | Main-axis alignment |
| `fullWidth` | `boolean` | `false` | Take 100% width |
| `fullHeight` | `boolean` | `false` | Take 100% height |
| `wrap` | `boolean \| 'wrap' \| 'nowrap' \| 'wrap-reverse'` | `false` | Flex wrap behavior |
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `columns` | `number \| ResponsiveValue<number>` | `1` | Number of columns |
| `gap` | `'xs' \| 'sm' \| 'md' \| 'lg' \| 'xl'` | `'md'` | Gap between items |
| `rowGap` | `'xs' \| 'sm' \| 'md' \| 'lg' \| 'xl'` | - | Vertical gap (overrides gap) |
| `columnGap` | `'xs' \| 'sm' \| 'md' \| 'lg' \| 'xl'` | - | Horizontal gap (overrides gap) |

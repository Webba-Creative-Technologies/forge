...
export function Footer({
  ...
  const defaultCopyright = `© ${currentYear} Tous droits réservés.`

  const renderLogo = () => {
    if (!logo) return null
    const logoElement = <div style={{ color: 'var(--text-primary)' }}>{logo}</div>
    if (logoHref) {
      return (
        <a href={logoHref} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', textDecoration: 'none' }}>
          {logoElement}
        </a>
      )
    }
